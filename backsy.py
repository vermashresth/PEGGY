...
class PopSenderReceiverRnnReinforce(nn.Module):
    ...
    def forward(self, sender_input, labels, receiver_input=None):
        ...
        if not normal:
          ...
          print("advices which are successful",mask.sum(), do_ask_advice.sum())
...
class MyReceiver(nn.Module):
    def __init__(self, game_size, feat_size, embedding_size,
        ...
        self.lin3 = nn.Linear(vocab_size, embedding_size, bias=False)
        self.lin4 = nn.Linear(vocab_size, embedding_size, bias=False)
    ...
    def forward(self, signal, x):
        ...
        h_s_1 = self.lin2(signal)
        h_s_2 = self.lin3(signal)
        ...
        # h_s is of size batch_size x embedding_size x n_heads
        h_s = torch.stack([h_s_1, h_s_2, h_s_3 ], 2)
        out = torch.bmm(emb, h_s)
        # out is of size batch_size x game_size x 1
        # out = out.squeeze(dim=-1)
        outs = out.transpose(1,2).transpose(0,1)

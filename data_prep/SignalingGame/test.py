...
def play(loader,players,opt,indexes_comp,reward_function,n_games,noise):
    ...
    for i_games in range(n_games):
        _,y,im_s,i_s,im_r,i_r,sims_im_s, sims_im_r=\
                    get_batch(opt,loader)
        if noise > 0:
...
def noise_game(opt,players,loader,idx_comp,reward_function,\
    ...
    if opt.epoch_test == -1:# end of training
        ...
        suffix=base+'_i%d.pt' % opt.epoch_test
...
def noise_pairs(opt,players,testSeed,n_pairs,base,noise):
    ...
    for p in range(n_pairs):
        ...
        one_hot_s1 = one_hot(s1, players.sender.given_vocab_size,cuda=opt.cuda)
        ...
        one_hot_s2 = one_hot(s2, players.sender.given_vocab_size,cuda=opt.cuda)
...
def test_noise(opt,n_g,out,game_seeds,chosen_seed=None,noise=0,\
    ...
    test_suffix = 'seed0_same0'
    if not out:
        indexes_comp = pickle.load(
            open( root1+"val_images_indexes_receiverseed0_same0", "rb" ) )
        loader = features_loader(root=opt.root, probs=opt.probs, norm=opt.norm,
    ...
    if chosen_seed==None:
        ...
        for idx_seed, seed in enumerate(seeds):
            base ='_sm%d_one%d_v%d_ours%d_seed%d_clip%d_lr%.4f_tau_s%d_same%d_noise0' %(opt.probs, opt.add_one, opt.vocab_size,
        ...
        best_base ='_sm%d_one%d_v%d_ours%d_seed%d_clip%d_lr%.4f_tau_s%d_same%d_noise0' %(opt.probs, opt.add_one, opt.vocab_size,
